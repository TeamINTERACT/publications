...
vic_sd_sf <- st_as_sf(vic_sd, coords = c("lon", "lat"))
vic_sd_sf <- st_set_crs(vic_sd_sf, 4326)
vic_sd_sf <- st_transform(vic_sd_sf, crs = 4326, "+proj=longlat +ellps=WGS84 +datum=WGS84")


vic_sd_sf1 <- vic_sd_sf %>%
                filter(st_intersects(., bbox_vic, sparse = FALSE)[1,])

vic_sd_sf1 <- vic_sd_sf %>%
                filter(st_intersects(., bbox_vic))

vic_sd_sf$test <- st_intersects(vic_sd_sf, bbox_vic)

table(vic_sd_sf$test)
...
ggplot() + geom_sf(data = vic_int_t, aes(colour = AAA))
